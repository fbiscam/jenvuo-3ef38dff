...
/** `bars` must be closed candles only (the forming candle excluded). */
export function computeSmcOverlay(bars: OhlcvBar[], currentPrice: number | null): SmcOverlay {
  const recent = bars.slice(-SMC_WINDOW).map((b) => ({
    t: b.time * 1000,
    o: b.open,
    h: b.high,
    l: b.low,
  ...
  if (recent.length < 10) {
    return { pivots: [], breaks: [], fvgs: [], orderBlocks: [], buySide: [], sellSide: [], trend: "undecided", windowStart: null };
  }
  ...
  const pivotsLabelled = structure.pivots.filter((p) => p.label.length === 2);

...
function SignalPage() {
  ...
  const handleStreamTick = useCallback((priceTick: number, tMs: number) => {
    ...
    // Only track & fire notifications when confidence ≥ 70% AND entry/SL/TP are valid.
    if ((plan.trade.confidence ?? 0) < 70) return;
    ...
    <div className="signal-page-root min-h-dvh w-full bg-[#FAFAFA] text-slate-900 antialiased" style={{ fontFamily: '"Google Sans", "Product Sans", "Roboto", system-ui, sans-serif', fontWeight: 400 }}>
      ...
      <div className="border-b border-zinc-100 bg-white/60">
        <div className="mx-auto max-w-[1600px] px-5 py-2 sm:px-6 sm:py-2.5 flex items-center gap-2 overflow-x-auto">
          <span className={`font-['Google_Sans','Product_Sans','Roboto',system-ui,sans-serif] text-[15px] font-normal normal-case tracking-normal text-zinc-900 shrink-0`}>Pair:</span>
          {XAU_PAIRS.map((p) => {
                ...
                onClick={() => {
                  ...
                  if (locked) {
                    toast.info("Multi-pair analysis is a Pro feature", {
                      description: "Free plan is limited to XAU/USD. Upgrade to unlock all pairs — gold crosses, FX majors, silver, platinum and crypto.",
                      action: { label: "Upgrade", onClick: () => (window.location.href = "/pricing") },
                    });
                    return;
                  }
              ...
              {t && plan && !marketClosed && (
                <div className="space-y-3">
                  ...
                  {(() => {
                    ...
                    return (
                      <div className="flex items-start gap-2 rounded-lg border border-amber-200 bg-amber-50 px-3 py-2 text-[10px] sm:text-[11px] text-amber-800 leading-snug">
                  ...
                  {(isBuy || isSell) && (t.confidence ?? 0) >= 70 && (

              ...
              {ALL_PAIRS.map((pair) => {
                ...
                return (
                  <button
                    ...
                    className={`rounded-md px-3 py-1.5 font-mono text-[11px] transition border ${on ? "bg-green-400 text-white border-green-400" : "bg-white text-zinc-500 border-zinc-200 hover:bg-zinc-50"}`}
                  >
                    {pair}
                  </button>
                );
  ...
  return (
    <label className="flex cursor-pointer items-start justify-between gap-4 rounded-lg border border-zinc-100 p-3 hover:bg-zinc-50">
      ...
      <button
        ...
        className={`relative mt-1 inline-flex h-5 w-9 shrink-0 items-center rounded-full transition ${checked ? "bg-green-400" : "bg-zinc-300"}`}
      >

    ...
    return (
      <div className="flex flex-1 flex-col items-center justify-center px-6 py-8 text-center">
        ...
        <p className="mt-1 max-w-[260px] text-[12px] text-zinc-500">
          The engine runs every 5 minutes. New A+ setups will land here automatically.
        </p>
      </div>
    );
  }
...
function UsageAnalytics({
  ...
  const periodSpendLabel =
    range === "24h" ? "Today spend" : range === "7d" ? "7 day spend" : "30 day spend";
        ...
        <div className="grid grid-cols-1 overflow-hidden rounded-lg border border-border sm:grid-cols-2 lg:grid-cols-3 [&>*]:border-border lg:[&>*:not(:nth-child(3n))]:border-r lg:[&>*:nth-child(-n+3)]:border-b sm:max-lg:[&>*:nth-child(odd)]:border-r sm:max-lg:[&>*:nth-child(-n+4)]:border-b max-sm:[&>*:not(:last-child)]:border-b">
          ...
          <div className={metricClass}>
            <div className="grid grid-cols-[minmax(0,1fr)_auto] items-center gap-2 text-sm text-foreground">
              <span className="truncate">Prompt caching: Hit rate</span>
          ...
          <div className={metricClass}>
            <div className="grid grid-cols-[minmax(0,1fr)_auto] items-center gap-2 text-sm text-foreground">
              <span className="truncate">{periodSpendLabel}</span>
            ...
            <div className="mt-4 flex items-center justify-between gap-3 text-sm text-foreground">
              ...
              <span className="tabular-nums">
                {fmtUsd2(totalSpent)} / {stats ? fmtUsd2(stats.allowance) : "$0.00"}
              </span>
            </div>
            <div className="mt-2 h-4 overflow-hidden rounded bg-muted">
              <div
                ...
                style={{
                  width: `${Math.min(100, Math.max(0, stats?.allowance ? (totalSpent / stats.allowance) * 100 : 0))}%`,
                }}
              />
            </div>
          </div>
                ...
                >
                  ...
                  <div className="relative z-10 flex h-full flex-col">
                    <div className="flex items-start justify-between gap-3">
                      <div
                        className={`flex h-11 items-center gap-2 rounded-md border px-2.5 ${model.featured ? "border-primary-foreground/30 bg-primary/75" : "border-border bg-background"}`}

...
function DashboardLayout() {
  ...
  return (
    <div className="min-h-dvh flex bg-[#FAFAFA] text-zinc-900 font-['Google_Sans','Product_Sans','Poppins',system-ui,sans-serif] antialiased jenvu-zoom-dashboard">
      ...
      >
        {/* Brand */}
        <div className={`flex h-14 shrink-0 items-center gap-2.5 ${sidebarCollapsed ? "justify-center px-2" : "px-4"}`}>
          <Link to="/" className="flex items-center gap-2.5 min-w-0">
            <img src="/favicon.png" alt="JENVU" className="h-7 w-7 shrink-0 rounded-md object-contain" />
            {!sidebarCollapsed && (
              <span className="truncate text-[22px] font-semibold tracking-tight leading-none" style={{ color: "#5e5e5e" }}>Jenvu</span>
            )}
          </Link>
          <button
            type="button"
          ...
          {NAV_GROUPS.map((group, gi) => (
            <div key={group.label} className={gi > 0 ? "mt-2 pt-3 border-t border-zinc-100" : ""}>
              {!sidebarCollapsed && group.label && (
                <div className="mb-1.5 px-2.5 text-[8px] font-normal uppercase tracking-wider text-[#9B9C9B]">
                  {group.label}
                </div>
              )}

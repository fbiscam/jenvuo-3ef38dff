...
const NAV_GROUPS: Array<{ label: string; items: TabItem[] }> = [
  {
    ...
    items: [
      ...
      { to: "/app", label: "Launch Upwork", icon: "auto_awesome" },
...
function DashboardLayout() {
  ...
  return (
    <div className={`flex min-h-screen bg-[#FAFAFA] text-zinc-900 font-['Google_Sans','Product_Sans','Poppins',system-ui,sans-serif] antialiased jenvu-zoom-dashboard`}>
      ...
      <aside
        className={`dashboard-sidebar-root max-lg:fixed max-lg:inset-y-0 max-lg:left-0 max-lg:z-50 lg:fixed lg:inset-y-0 lg:left-0 flex min-h-0 shrink-0 flex-col overflow-hidden border-r border-zinc-200 bg-[#F5F5F5] transition-[width,transform] duration-200 ease-out
      ...
      >
        ...
        {/* Brand */}
        <div className={`flex h-11 shrink-0 items-center gap-2.5 bg-[#F5F5F5] ${sidebarCollapsed ? "justify-center px-2" : "px-4"}`}>
        ...
        <nav className="sidebar-hover-scroll flex-1 min-h-0 overflow-y-auto overflow-x-hidden bg-[#F5F5F5] px-2 py-2">
        ...
        <div className={`mt-auto shrink-0 flex items-center border-t border-zinc-200 bg-[#F5F5F5] py-2 ${sidebarCollapsed ? "justify-center px-2" : "justify-between pl-3 pr-2"}`}>
          {!sidebarCollapsed && (
            <button
              type="button"
              onClick={signOut}

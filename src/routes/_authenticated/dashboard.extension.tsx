...
function ExtensionPage() {
  ...
  const copyListedKey = async (key: ExtensionKeyRow) => {
    ...
    toast.error(
      "For security, older full keys cannot be shown again. Create a new key to copy it.",
    );
  };
  ...
  const onDownload = async () => {
    ...
    try {
      ...
      a.download = "jenvu-extension-v1.9.19.zip";
      a.rel = "noopener";
  ...
  const canCreate = Boolean(
    access?.active && access.plan !== "free" && activeKeys.length < access.keyLimit,
  );
      ...
      {access && (
        <div className="mt-5 grid gap-3 border-y border-zinc-200 py-4 sm:grid-cols-3">
          <div>
            ...
            <div className="mt-1 text-sm font-medium text-zinc-900">{access.planName}</div>
          </div>
          <div>
            <div className="text-[11px] text-zinc-500">Active API keys</div>
            <div className="mt-1 text-sm font-medium tabular-nums text-zinc-900">
              {activeKeys.length} / {access.keyLimit}
            </div>
          </div>
          <div>
            <div className="text-[11px] text-zinc-500">AI wallet</div>
            <div
              className={`mt-1 text-sm font-medium tabular-nums ${access.balance < 0.02 ? "text-rose-600" : "text-zinc-900"}`}
            >
              ${access.balance.toFixed(2)} / ${access.wallet.toFixed(2)}
            </div>
          </div>
        </div>
      )}
      ...
      {freshKey && (
        <div className="mt-5 rounded-lg border border-amber-300 bg-amber-50 p-3">
          ...
          <div className="mt-2 flex items-center gap-2">
            <code
              className={`${MONO} flex-1 truncate rounded border border-amber-200 bg-white px-2 py-1.5 text-[12px] text-zinc-900`}
            >
              {freshKey}
            </code>
            <button
  ...
  -d '{"model":"claude-sonnet-4.5","messages":[{"role":"user","content":"XAU/USD bias today?"}]}'`}</pre>
        ...
        <div className="mt-5">
          <div className="mb-4 flex items-end justify-between gap-3">
            <div>
              ...
              <p className="mt-1 text-[12px] text-zinc-500">
                Your four newest extension keys, newest first.
              </p>
            </div>
            <span className="shrink-0 text-[12px] tabular-nums text-zinc-500">
              {displayedKeys.length} shown
            </span>
          </div>
              ...
              >
                <div className="min-w-0">
                  ...
                  <div className="flex min-w-0 items-center gap-1 rounded-md border border-zinc-200 bg-zinc-50 px-2.5 py-2">
                    <code className={`${MONO} min-w-0 flex-1 truncate text-[12px] text-zinc-900`}>
                      {k.key_prefix}••••••••••
                    </code>
                    <button
                      type="button"
                      title={
                        freshKey?.startsWith(k.key_prefix)
                          ? "Copy full API key"
                          : "Full key is no longer available"
                      }
                ...
                <div className="min-w-0">
                  ...
                  <div className={`${MONO} truncate text-[12px] text-zinc-500`}>v1.9.19</div>
                </div>
          ...
          {!loading && keys.length === 0 && (
            <div className="flex flex-col items-center justify-center py-16 text-center">
              ...
              <div className="mt-6 text-[15px] font-medium text-zinc-900">
                Can&apos;t find your API keys here?
              </div>
              <p className="mt-2 max-w-md text-[13px] leading-relaxed text-zinc-500">
                This list only shows keys created for the Jenvu extension. Create a new API key
                above, paste it into the extension and it signs in instantly.
              </p>
              <button
          ...
          {!loading && keys.length > 0 && (
            <div className="mt-3 text-[12px] text-zinc-500">
              Showing {displayedKeys.length} of {keys.length} key(s) · {activeKeys.length} of{" "}
              {access?.keyLimit ?? 0} active.
            </div>
          )}
        </div>
      )}
        ...
        <div className="mt-5">
          ...
          <div className="grid gap-4 border-b border-zinc-100 py-5 md:grid-cols-[minmax(0,1.8fr)_0.8fr_0.8fr_auto] md:items-center">
            ...
            <div className="flex items-center gap-2 text-[13px] text-zinc-800">
               <span className={MONO}>v1.9.19</span>
              <span className="rounded-full bg-emerald-50 px-2 py-0.5 text-[11px] font-medium text-emerald-700">
                Latest
              </span>
            </div>
          ...
          <p className="mt-4 text-[12px] text-zinc-500">
             ZIP package · Chrome developer mode · Version 1.9.19
          </p>
        </div>
      )}
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-zinc-900/30 p-4">
          <div className="w-full max-w-md rounded-xl border border-zinc-200 bg-white p-5 shadow-xl">
            ...
            <p className="mt-1 text-[13px] text-zinc-500">
              Give the key a name so you can recognise the device using it.
            </p>
            <input
              value={name}
      ...
      {deleteTarget && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-zinc-900/30 p-4">
          ...
          >
            <div className="flex items-start justify-between gap-3">
              <div id="delete-key-title" className="text-[15px] font-medium text-zinc-900">
            ...
            <p className="mt-2 text-[13px] leading-relaxed text-zinc-500">
              <span className="font-medium text-zinc-800">{deleteTarget.name}</span> will be removed
              from your account and database. This cannot be undone.
            </p>
            <div className="mt-5 flex justify-end gap-2">
              <button
                type="button"
                onClick={() => setDeleteTarget(null)}
                disabled={deleting}
                className="rounded-full px-4 py-2 text-[13px] text-zinc-700 hover:bg-zinc-100 disabled:opacity-60"
              >
                Cancel
              </button>
              <button
                ...
                onClick={onDelete}
                disabled={deleting}
                className="rounded-full bg-rose-600 px-4 py-2 text-[13px] font-medium text-white hover:bg-rose-700 disabled:opacity-60"
              >

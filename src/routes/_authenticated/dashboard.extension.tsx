...
function ExtensionPage() {
  ...
  const onDownload = async () => {
    setDownloading(true);
    try {
      const res = await fetch("/jenvu-extension-v1.9.2.zip", { cache: "no-store" });
      if (!res.ok) throw new Error(`Download failed: ${res.status}`);
      const blob = await res.blob();
      const a = document.createElement("a");
      a.href = URL.createObjectURL(blob);
      a.download = "jenvu-extension-v1.9.2.zip";
      a.click();
    ...
    <div className="w-full px-1 py-2 sm:px-2 sm:py-4">
      ...
      {!loading && access && capabilities.extensionAi && !capabilities.seniorReview && (
        <div className="mt-5 rounded-lg border border-blue-200 bg-blue-50 p-4 text-[13px] text-blue-900">
          Your Pro plan includes GPT-6 Astra primary analysis for XAU/USD. Elite or Ultra unlocks multi-pair scanning and mandatory Claude senior review.
        </div>
      )}
          ...
          displayedKeys.map((k) => (
            ...
            >
              ...
              <div className="min-w-0">
                ...
                <div className={`${MONO} truncate text-[12px] text-zinc-500`}>v1.9.2</div>
              </div>
        ...
        <div className="mt-5">
          ...
          <div className="grid gap-4 border-b border-zinc-100 py-5 md:grid-cols-[minmax(0,1.8fr)_0.8fr_0.8fr_auto] md:items-center">
            ...
            <div className="flex items-center gap-2 text-[13px] text-zinc-800">
              <span className={MONO}>v1.9.2</span>
          ...
          <p className="mt-4 text-[12px] text-zinc-500">ZIP package · Chrome developer mode · Version 1.9.2</p>
        </div>
      )}
      ...
      {showGuide && (
        <div className="mt-6 rounded-xl border border-zinc-200 bg-zinc-50 p-5">
          ...
          <p className="mt-3 text-[12px] leading-relaxed text-zinc-600">AI chat and primary-only analysis cost $0.03 per request. Each completed senior-reviewed analysis costs $0.20. Pro includes GPT-6 Astra for XAU/USD; Elite and Ultra add multi-pair scanning and mandatory Claude review.</p>

...
async function handle({ request }: { request: Request }) {
  ...
  return extJson({ ok: true, user: { id: auth.userId, email, plan }, key: { id: auth.keyId, name: auth.name }, access: { keyLimit: access.keyLimit, wallet: access.wallet, balance: access.balance, aiEnabled: access.allowed, seniorReview: access.seniorReview, capabilities: access.capabilities } })
}

  ...
  server: {
    handlers: {
      POST: async ({ request }) => {
        ...
        // IndexNow accepts batched URLs — one call
        const indexnow = await submitToIndexNow([...urls, `${BASE_URL}/sitemap.xml`]);

        // Google indexing API is per-URL; run in parallel with mild concurrency
      ...
      GET: async ({ request }) => {
        // Allow GET for easy manual trigger with ?secret=...

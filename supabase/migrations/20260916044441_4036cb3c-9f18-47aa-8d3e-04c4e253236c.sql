CREATE OR REPLACE FUNCTION public.cancel_user_plan(_user_id uuid)
RETURNS jsonb
LANGUAGE plpgsql
SECURITY DEFINER
...
BEGIN
  ...
  IF _old_plan IS NULL THEN
    SELECT plan INTO _old_plan FROM public.profiles WHERE id = _user_id;
    _old_plan := COALESCE(_old_plan, 'free');
    _old_status := 'none';
  END IF;
  ...
  INSERT INTO public.user_subscriptions (user_id, plan_id, status, is_trial, trial_ends_at, current_period_end, updated_at)
  VALUES (_user_id, 'free', 'cancelled', false, NULL, now(), now())
